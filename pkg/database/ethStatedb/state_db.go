...
type StateDB struct {
	...
	stateObjectsPending map[types.Address]struct{} // State objects finalized but not yet written to the trie
...
func New(root types.Hash, db basedb.Database) (*StateDB, error) {
	...
	return &StateDB{
		...
		stateObjectsPending: make(map[types.Address]struct{}),
...
func (s *StateDB) Reset(root types.Hash) error {
	...
	s.stateObjectsPending = make(map[types.Address]struct{})
...
// AddressExist reports whether the given account address exists in the state.
...
func (s *StateDB) createObject(addr types.Address) (newobj, prev *stateObject) {
	...
	newobj = newObject(s, addr, model.Account{})
...
func (s *StateDB) Copy() *StateDB {
	...
	state := &StateDB{
		...
		stateObjects:        make(map[types.Address]*stateObject),
		stateObjectsPending: make(map[types.Address]struct{}, len(s.stateObjectsPending)),
...
func (s *StateDB) Commit(deleteEmptyObjects bool) (types.Hash, error) {
	...
	for addr := range s.stateObjectsDirty {
		if obj := s.stateObjects[addr]; !obj.deleted {
			...
			// TODO kvs
			if obj.kvs != nil && obj.dirtyKvs {
				kvBytes, err := rlp.EncodeToBytes(obj.kvs)
				if err != nil {
					return types.EmptyHash, err
				}
				s.db.TreeDB().InsertBlob(types.BytesToHash(obj.CodeHash()), kvBytes)
				obj.dirtyKvs = false
			}
...
func (s *StateDB) GetKVS(addr types.Address) *linkedHashMap.LinkedHashMap {
	...
	return stateObject.KVS()
}

func (s *StateDB) GetKVNP(addr types.Address, namespace string) (*linkedHashMap.LinkedHashMap, error) {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		return nil, errors.New("stateObject is empty")
	}
	return stateObject.KVNP(namespace)
}
...
func (s *StateDB) GetKV(addr types.Address, namespace string, key string) (interface{}, error) {
	...
	return stateObject.KV(namespace, key)
}

func (s *StateDB) AddKV(addr types.Address, namespace string, key string, val interface{}) error {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		return errors.New("stateObject is empty")
	}
	stateObject.AddKV(namespace, key, val)
	return nil
}
...
func (s *StateDB) KVExist(addr types.Address, namespace string, key string) bool {
	...
	return stateObject.KVExist(namespace, key)
}
