...
type (
	...
	kvsChange struct {
		account     *types.Address
		prevKvsHash []byte
		prevKvs     *linkedHashMap.LinkedHashMap
	}
)
...
func (ch kvsChange) revert(s *StateDB) {
	s.getStateObject(*ch.account).setKVS(types.BytesToHash(ch.prevKvsHash), ch.prevKvs)
}

func (ch kvsChange) dirtied() *types.Address {
	return ch.account
}
